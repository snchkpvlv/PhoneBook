...
std::string currentFile = "contacts.txt";
...
int main() {
    ...
    if (FileStorage::fileExists(currentFile)) {
        ...
        std::cout << "Файл " << currentFile << " не найден. Будет создан новый справочник.\n";
    }
...
void saveToFile() {
    if (FileStorage::saveToFile(contacts, currentFile)) {
        std::cout << "\nДанные успешно сохранены в файл: " << currentFile << "\n";
...
void loadFromFile() {
    ...
    if (FileStorage::loadFromFile(loadedContacts, currentFile)) {
        contacts = loadedContacts;
        std::cout << "\nДанные успешно загружены из файла: " << currentFile << "\n";
